<?php
class Devinc_Ajaxcartx_Model_Ajaxcartx extends Mage_Core_Model_Abstract
{    
	//add quote layout messages to checkout session
    public function loadQuoteMessages()
    {
		if (Mage::helper('ajaxcartx')->getMagentoVersion()>1420 && Mage::helper('ajaxcartx')->getMagentoVersion()<1800) {    
			$messages = array();
	...
	public function getAjaxCartQtysJs()
	{
		...
		if (count($items)){			
			foreach($items as $item){
				...
				$minQty = Mage::helper('ajaxcartx')->getMinimumProductQty($sidebarProduct);
				$maxQty = Mage::helper('ajaxcartx')->getMaximumProductQty($sidebarProduct);
				$incQty = Mage::helper('ajaxcartx')->getQtyIncrements($sidebarProduct);
