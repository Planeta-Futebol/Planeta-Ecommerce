...
class Devinc_Ajaxcartx_CartController extends Mage_Checkout_CartController
{
	...
	protected function _goBack()
    {
		...
		if (!isset($this->_result['error'])) {
			...
			$this->_result['update_section']['html_cart'] = Mage::helper('ajaxcartx')->getCartHtml($this);
			$this->_result['update_section']['html_cart_link'] = Mage::helper('ajaxcartx')->getCartLinkHtml($this);
			$this->_result['update_section']['html_wishlist_link'] = Mage::helper('ajaxcartx')->getWishlistLinkHtml($this);
			...
			if(strpos($refererUrl, 'checkout/cart') !== false || $this->getRequest()->getParam('is_cart_page', false)) {
				...
				$this->_result['update_section']['html_cart_page'] = Mage::helper('ajaxcartx')->getCartPageHtml($this);
				$this->_result['update_section']['html_ajaxcartx_js'] = Mage::getModel('ajaxcartx/ajaxcartx')->getAjaxCartQtysJs();
			} else {
			 	$session = $this->_getSession();		
				$messages = $session->getMessages();
				foreach( $messages->getItems() as $message ){
