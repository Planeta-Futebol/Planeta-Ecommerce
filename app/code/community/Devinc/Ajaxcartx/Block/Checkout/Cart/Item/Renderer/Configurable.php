 ...
 */
class Devinc_Ajaxcartx_Block_Checkout_Cart_Item_Renderer_Configurable extends Mage_Checkout_Block_Cart_Item_Renderer_Configurable
{
    ...
    public function getOptionList()
    {
		...
		if (Mage::getSingleton('customer/session')->getAjaxCartAction() == 'cart' && ($browserInfo['name']!='msie' || ($browserInfo['name']=='msie' && $browserInfo['version']>7))) {
			...
			foreach($jsonConfigDecoded->attributes as $attribute) {
				...
				$onChange = (count($_configurable->getAllowAttributes())==($i+1)) ? 'onchange="ajaxcartx.updateItemOptions(this);"' : '';				
				$selectHtml = '';
