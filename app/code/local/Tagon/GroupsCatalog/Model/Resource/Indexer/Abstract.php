...
abstract class Tagon_GroupsCatalog_Model_Resource_Indexer_Abstract extends Mage_Index_Model_Resource_Abstract
{
    /**
    ...
    /**
     * Array of all valid group ids plus Tagon_GroupsCatalog_Helper_Data::USE_NONE
     *
     * @var array $_groupIds
     */
    ...
    {
        parent::__construct();
        $this->_helper = Mage::helper('tagon_groupscatalog');
        $this->_initStores();
        $this->_initGroupIds();
    }
    ...
    protected function _initGroupIds()
    {
        ...
        $this->_groupIds[] = Tagon_GroupsCatalog_Helper_Data::USE_NONE;
    }
    ...
    {
        return 'Tagon_GroupsCatalog::' . $this->_getEntityTypeCode();
    }
    ...
    /**
     * @return Tagon_GroupsCatalog_Model_Resource_Setup
     */
    protected function _getSetupModel()
    {
        return Mage::getResourceModel(
            'tagon_groupscatalog/setup',
            'tagon_groupscatalog_setup'
        );
    }
    ...
    protected function _checkAttribute()
    {
        ...
        if (! $attributeChecked) {
            ...
            $select = $this->_getReadAdapter()->select()
                ...
                ->where('attribute_code=?', Tagon_GroupsCatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE);
    ...
    protected function _reindexEntity($event = null, array $limitToGroupIds = null)
    {
        ...
        $attribute = Mage::getSingleton('eav/config')->getAttribute(
            $this->_getEntityTypeCode(), Tagon_GroupsCatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE
        );
        $select = $this->_getReadAdapter()->select()
    ...
    {
        // Entities that don't have a value for the groupscatalog_groups attribute
        if (null === $row['group_ids']) {
            $row['group_ids'] = Tagon_GroupsCatalog_Helper_Data::USE_DEFAULT;
            $row['store_id'] = Mage::app()->getStore(Mage_Core_Model_Store::ADMIN_CODE)->getId();
        }

        if (Tagon_GroupsCatalog_Helper_Data::USE_DEFAULT == $row['group_ids']) {
            ...
            $row['orig_group_ids'] = Tagon_GroupsCatalog_Helper_Data::USE_DEFAULT;
            ...
            if (in_array(Tagon_GroupsCatalog_Helper_Data::USE_NONE, $row['group_ids'])) {
                $row['group_ids'] = array();
            }
    ...
    )
    {
        foreach (array_diff($this->_frontendStoreIds, $storesHandled) as $storeId) {
            if (Tagon_GroupsCatalog_Helper_Data::USE_DEFAULT === $entityDefaultGroupsWithoutMode) {
