...
class Tagon_GroupsCatalog_Helper_Migration
    extends Mage_Core_Helper_Abstract
{
    ...
    /**
     * @var Tagon_GroupsCatalog_Model_Resource_Migration
     */
    protected $_resource;
    ...
    {
        $file = Mage::getBaseDir('etc') . DS . 'modules' . DS . 'Tagon_GroupsCatalog.xml';
        $io = new Varien_Io_File();
        if (!$io->fileExists($file)) {
            $message = Mage::helper('tagon_groupscatalog')->__(
                "The file app/etc/modules/Tagon_GroupsCatalog.xml doesn't exist."
            );
            Mage::throwException($message);
        }
        $xml = simplexml_load_file($file);
        if (in_array((string) $xml->modules->Tagon_GroupsCatalog->active, array('true', '1'), true)) {
            if (!$io->isWriteable($file)) {
                $message = Mage::helper('tagon_groupscatalog')->__(
                    'The file app/etc/modules/Tagon_GroupsCatalog.xml is not writable.<br/>' .
                    'Please fix it and flush the configuration cache, or deactivate the module manually in that file.'
                );
                Mage::throwException($message);
            }
            $xml->modules->Tagon_GroupsCatalog->active = 'false';
            $xml->asXML($file);
    ...
    protected function _migrateSystemConfig()
    {
        ...
        foreach (Mage::app()->getStores(true) as $store) {
            ...
            $settings['tagon_groupscatalog/general/is_active'] = $isActive;

            $hideFromGroups = $this->_getConfigValueWithDefault(
                $store, 'catalog/groupscatalog/default_product_groups', $useNone
            );
            $settings['tagon_groupscatalog/general/product_default_hide'] = $hideFromGroups;

            $hideFromGroups = $this->_getConfigValueWithDefault(
                $store, 'catalog/groupscatalog/default_category_groups', $useNone
            );
            $settings['tagon_groupscatalog/general/category_default_hide'] = $hideFromGroups;
    ...
    {
        $mode = Tagon_GroupsCatalog_Helper_Data::MODE_SHOW_BY_DEFAULT;

        // Default settings
        $config->saveConfig('tagon_groupscatalog/general/product_mode', $mode, 'default', 0);
        $config->saveConfig('tagon_groupscatalog/general/category_mode', $mode, 'default', 0);
        $config->saveConfig('tagon_groupscatalog/general/auto_refresh_block_cache', 1, 'default', 0);
        $config->deleteConfig('tagon_groupscatalog/general/product_default_show', 'default', 0);
        $config->deleteConfig('tagon_groupscatalog/general/category_default_show', 'default', 0);

        // Remove any website scope setting
        foreach (Mage::app()->getWebsites() as $website) {
            $config->deleteConfig(
                'tagon_groupscatalog/general/product_mode', 'websites', $website->getId()
            );
            $config->deleteConfig(
                'tagon_groupscatalog/general/category_mode', 'websites', $website->getId()
            );
            $config->deleteConfig(
                'tagon_groupscatalog/general/product_default_show', 'websites', $website->getId()
            );
            $config->deleteConfig(
                'tagon_groupscatalog/general/category_default_show', 'websites', $website->getId()
            );
            $config->deleteConfig(
                'tagon_groupscatalog/general/product_default_hide', 'websites', $website->getId()
            );
            $config->deleteConfig(
                'tagon_groupscatalog/general/category_default_hide', 'websites', $website->getId()
            );
            $config->deleteConfig(
                'tagon_groupscatalog/general/product_mode', 'websites', $website->getId()
            );
    ...
    protected function _updateConfigStoreScope(Mage_Core_Model_Config $config, $scope, $scopeId, $settings, $defaults)
    {
        ...
        if ('default' !== $scope) {
            $config->deleteConfig('tagon_groupscatalog/general/product_mode', $scope, $scopeId);
            $config->deleteConfig('tagon_groupscatalog/general/category_mode', $scope, $scopeId);
            $config->deleteConfig('tagon_groupscatalog/general/product_default_show', $scope, $scopeId);
            $config->deleteConfig('tagon_groupscatalog/general/category_default_show', $scope, $scopeId);
        }
    }
     ...
     *
     * @return Tagon_GroupsCatalog_Model_Resource_Migration
     */
    protected function _getResource()
    {
        if (is_null($this->_resource)) {
            $this->_resource = Mage::getResourceModel('tagon_groupscatalog/migration');
        }
    ...
    protected function _migrateProductSettings()
    {
        ...
        $newAttribute = Mage::getSingleton('eav/config')
                ->getAttribute(
                    Mage_Catalog_Model_Product::ENTITY, Tagon_GroupsCatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE
                );
        ...
        $dataObj = new Varien_Object(array(
            ...
            'attributes_data' => array(Tagon_GroupsCatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE => true),
            //'store_id' => 0 // not used by GroupsCatalog indexer
        ));
    ...
    protected function _migrateCategorySettings()
    {
        ...
        $newAttribute = Mage::getSingleton('eav/config')
                ->getAttribute(
                    Mage_Catalog_Model_Category::ENTITY, Tagon_GroupsCatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE
                );
    ...
    protected function _removeFiles()
    {
        $message = $this->__('Remove the following files and directories to complete the uninstall:<br/>%s',
            implode('<br/>', array(
                'app/etc/modules/Tagon_GroupsCatalog.xml',
                'app/code/community/Tagon/GroupsCatalog/',
                'app/locale/de_DE/Tagon_GroupsCatalog.csv',
                'app/locale/en_US/Tagon_GroupsCatalog.csv',
                'app/locale/fr_FR/Tagon_GroupsCatalog.csv',
                'app/locale/nl_NL/Tagon_GroupsCatalog.csv'
            )
        ));
        Mage::throwException($message);
    }
