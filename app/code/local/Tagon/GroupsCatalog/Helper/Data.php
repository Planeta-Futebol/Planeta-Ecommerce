...
class Tagon_GroupsCatalog_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const XML_CONFIG_PRODUCT_MODE = 'tagon_groupscatalog/general/product_mode';
    const XML_CONFIG_CATEGORY_MODE = 'tagon_groupscatalog/general/category_mode';
    const XML_CONFIG_PRODUCT_DEFAULT_PREFIX = 'tagon_groupscatalog/general/product_default_';
    const XML_CONFIG_CATEGORY_DEFAULT_PREFIX = 'tagon_groupscatalog/general/category_default_';
    const XML_CONFIG_DISABLED_ROUTES = 'global/tagon_groupscatalog/disabled_on_routes';

    const HIDE_GROUPS_ATTRIBUTE = 'groupscatalog_groups';
    const HIDE_GROUPS_ATTRIBUTE_STATE_CACHE = 'groupscatalog_groups_state_cache';
    const CUSTOMER_GROUP_CACHE_TAG = 'groupscatalog_customer_group';
    ...
    /**
     * If set to false groupscatalog filtering is skipped
     *
     * @var bool|null
     */
    ...
    /**
     * Return a configuration setting from within the tagon_groupscatalog/general section.
     *
    ...
    {
        return Mage::getStoreConfig('tagon_groupscatalog/general/' . $field, $store);
    }
    ...
    public function isEntityVisible(Mage_Catalog_Model_Abstract $entity, $customerGroupId = null)
    {
        ...
        if (!is_array($groupIds) && !is_string($groupIds)) {
            ...
            $visibility = Mage::getResourceSingleton('tagon_groupscatalog/filter')
    ...
    public function getIndexTableByEntityType($entityType)
    {
        ...
        switch ($entityType->getEntityTypeCode()) {
            ...
            case Mage_Catalog_Model_Product::ENTITY:
                $table = 'tagon_groupscatalog/product_index';
                break;

            case Mage_Catalog_Model_Category::ENTITY:
                $table = 'tagon_groupscatalog/category_index';
                break;
        }
        return $table;
    }
    ...
    public function getGroupNamesAsString(array $value)
    {
        ...
        $key = array_search(Tagon_GroupsCatalog_Helper_Data::USE_DEFAULT, $value);
        if (false !== $key) {
            $list[] = $this->__(Tagon_GroupsCatalog_Helper_Data::LABEL_DEFAULT);
            unset($value[$key]);
        }
        $key = array_search(Tagon_GroupsCatalog_Helper_Data::USE_NONE, $value);
        if (false !== $key) {
            $list[] = $this->__(Tagon_GroupsCatalog_Helper_Data::LABEL_NONE);
            unset($value[$key]);
        }
        if (count($value)) {
            ...
            $groups->initCache(Mage::app()->getCache(), 'groupscatalog', array(
