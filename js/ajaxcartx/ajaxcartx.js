...
var AjaxcartxTools = Class.create();
AjaxcartxTools.prototype = {
	initialize: function(){
	...
	showPopup: function(id,action){
		...
		//start petal loader		
		if (id=='ajaxcartx-loading') {		
			this.runLoader = true;			
			ajaxcartxTools.initPetal('#petal-1', 0.10, 90, false);    
			ajaxcartxTools.initPetal('#petal-2', 0.05, 90, true);   
			ajaxcartxTools.initPetal('#petal-3', 0.10, 0, true); 
		}	
					
		//display success buttons
		if (id=='success' && action){
			this.countdownTimer(ajaxcartx.autohideTime);
			jQueryAC('#success-'+action+'-button').css({'display':'inline-block'});
			if (action=='cart' && ajaxcartx.showCheckoutButton == 1) {
		...
		//run scale up effect
		if (id!='ajaxcartx-loading') {		
			if (!ajaxcartx.isMobile && !ajaxcartx.isTablet && (!ajaxcartxTools.browserTypeIE || (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion == 10))) {	
		 		jQueryAC('#'+id+'-popup-content').addClass('grow');
		 		setTimeout(function(){ jQueryAC('#'+id+'-popup-content').addClass('shrink-to-normal'); }, 200);
		 	} else if (ajaxcartx.isMobile || ajaxcartx.isTablet) {
			...
			if (ajaxcartx.showNotificationBkg == 1 && ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion <= 8) {					
		    	jQueryAC('#ac-popup-wrapper-bkg').show();
			} else if (ajaxcartx.showNotificationBkg == 1) {				
		    	jQueryAC('#ac-popup-wrapper-bkg').show();
		    	jQueryAC('#ac-popup-wrapper-bkg').animate({'backgroundColor': 'rgba(' + ajaxcartx.notificationWrapperBkg + ', 0.6)'},ajaxcartx.successPopupDelay);	
			}
		}
	},

	hidePopup: function(id, instant){	
		if (this.runHidePopup && (!instant || (instant && !ajaxcartx.ajaxCartRunning && !ajaxcartxLogin.ajaxCartLoginRunning))) {			
			if (ajaxcartx.showNotificationBkg == 1 && id!='ajaxcartx-loading' && (id!='options' || (id=='options' && instant) || (id=='options' && ajaxcartx.showNotification != 1)) && (id!='ajaxcartx-login' || (id=='ajaxcartx-login' && instant) || (id=='ajaxcartx-login' && ajaxcartx.showNotification != 1))) {	
			...
			if (id != 'ajaxcartx-loading') {
				if (instant) {				
					...
					if (id == 'options') {
						jQueryAC('#ajaxcartx-options').html('');
					}
				} else {
					//hide popup
			...
			if (id == 'success') {
				var adjustDelay = 10;
				if (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion <= 9) {
					adjustDelay = 50;
				}
				setTimeout(function() { 
					...
					ajaxcartxTools.deactivateTimer();
					
					//clear popup messages
					jQueryAC('#ajaxcartx-layout-messages').html(''); 
					jQueryAC('#ajaxcartx-custom-block').html(''); 
					jQueryAC('#ajaxcartx-product-images').html(''); 
				}, (ajaxcartx.successPopupDelay-adjustDelay));
			}	
		} else {
			ajaxcartxTools.runHidePopup = true;
		}
	},

	setLoadWaiting: function(step, enabled) {
		if (step == 'ajaxcartx-loading'){
			if (!ajaxcartx.addToCartButton){
				if (enabled) {
					this.showPopup('ajaxcartx-loading', false);				
				} else {
					this.hidePopup('ajaxcartx-loading', false);
				...
				if (enabled) {
					var cartButton = jQueryAC(ajaxcartx.addToCartButton);
					if (cartButton) {	   
						cartButton.replaceWith(ajaxcartx.ajaxCartLoadingHtml); 
					}	
				} else {
					if (jQueryAC('#ajax-cart-please-wait').length > 0 && (ajaxcartx.hasError || jQueryAC('#product_addtocart_form').length > 0)) {	   
						jQueryAC('#ajax-cart-please-wait').replaceWith(ajaxcartx.addToCartButton.outerHTML);
					}					
					ajaxcartx.hasError = false;
					ajaxcartx.addToCartButton = false;
					//enable all buttons, inputs, links etc.
					jQueryAC('#ac-popup-top-bkg').hide();
				}		
			}
	...
	resetLoadWaiting: function(step){
		...
		ajaxcartx.ajaxCartRunning = false;
	},		
	...
	initPetal: function(id, speed, startDegree, rotateClockwise) {
		if (this.runLoader && jQueryAC(id).length>0) {
		    ...
		    var r = 30;	    
		    var xcenter = parseInt(jQueryAC('#ajaxcartx-loading-popup-content').css('width'))/2; // Usable window width
			var ycenter = parseInt(jQueryAC('#ajaxcartx-loading-popup-content').css('height'))/2; // Usable window height
		    ...
		    jQueryAC(id).animate({
		        ...
		        ajaxcartxTools.initPetal(id, speed, startDegree, rotateClockwise)
		    });	   
		}
	},
	
	resetPetals: function() {
		ajaxcartxTools.runLoader = true;
		ajaxcartxTools.initPetal('#petal-1', 0.10, 90, false);    
		ajaxcartxTools.initPetal('#petal-2', 0.05, 90, true);   
		ajaxcartxTools.initPetal('#petal-3', 0.10, 0, true); 
		ajaxcartxTools.runLoader = false;
	},
	
	//automatically close success popup functions	
	countdownTimer: function(closeTime){
	    jQueryAC("#countdownToClose").html(" (" + ajaxcartx.autohideTime + ")");
	    
	    var closeValue = parseInt(closeTime) - 1;
	    this.countdown = setInterval(function(){
	    	jQueryAC("#countdownToClose").html(" (" + closeValue + ")");
	    	if (closeValue == 0) {
	    		clearInterval(ajaxcartxTools.countdown); 
	    		ajaxcartxTools.hidePopup('success', true);
	    	} else {
	    		closeValue--;
	    	}
	    }, 1000);
	...
	jump: function(productId, block, button) {
		// && (!ajaxcartxTools.browserTypeIE || (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion != 9))
		if (!ajaxcartx.itemDropped && ajaxcartx.jump == 1 && block.length > 0) {
			if (!button) {
			...
			if (productImageContainer.length>0) {
				...
				if ((window.opera && opera.version()<13) || (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion <= 9)) {
				...
				setTimeout(function() {		
				    ...
				    } else if (block.hasClass('ac-header')) {
				    	var yMiddle = yEnd-(blockSidebarOffset.top-jQueryAC(document).scrollTop());
				    } else {
				    	var yMiddle = yEnd-blockSidebarOffset.top;
				    }

				    ajaxcartx.dragContainer = clone;			    
				    clone.animate(
						...
						function() {
							...
							if (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion >= 11) {
				...
				ajaxcartx.updateWindow.dispatchJump(clone);
			}
		}
	},
	...
	updateSection: function(content, id) {
        ...
        var updateWindow = ajaxcartx.updateWindow;
        if (id == 'ajaxcartx-login-popup-content' || id == 'ajaxcartx-options' || id == ajaxcartx.comparePopup) {
        	updateWindow = window;
        }
        ...
        if (js_scripts != null && js_scripts != ''){
			for (var i=0; i< js_scripts.length; i++){
				if (typeof(js_scripts[i]) != 'undefined'){
					...
					//reinitialize the ajaxcartx js for the compare popup as well (not just the parent window); runs when ajax login is called from the compare popup
					if (id == 'ajaxcartx-qty-js' && window.opener!=null) {
...
var ajaxcartxTools = new AjaxcartxTools();

var AjaxcartxLogin = Class.create();
AjaxcartxLogin.prototype = {
	//main functions
	...
	save: function(transport){
		...
		if (response.error){		
			ajaxcartxTools.resetLoadWaiting('ajaxcartx-loading');
			ajaxcartxTools.resetLoadWaiting('login-mini');
			ajaxcartxLogin.ajaxCartLoginRunning = false;
        ...
        //update ajaxcartx blocks
		if (response.update_section) {
			if (response.update_section.html_login) {
				ajaxcartxTools.updateSection(response.update_section.html_login, 'ajaxcartx-login-popup-content');
			 ...
			 //    	}
    		    ...
    		    //save response in variable for later use in ajaxcartx class to update all the blocks at once, after the product has been added to wishlist
				this.loginPostResponse = response;
				this.isLoggedIn = true;
				if (window.opener != null && window.location.href.indexOf('catalog/product_compare/index/')!=-1 && jQueryAC('#'+ajaxcartx.comparePopup).length > 0) {
					window.opener.ajaxcartxLogin.isLoggedIn = true;
				}
				ajaxcartx.addToWishlist(this.addToWishlistUrl, false);	
								
				//update wishlist urls
				var wishlistElements = ajaxcartx.updateWindow.document.getElementsByTagName('a');
				...
				for ( var i = 0; i<wishlistElements.length; i++ ) {
					if (typeof wishlistElements[i].getAttribute("href") != 'undefined' && wishlistElements[i].getAttribute("href")!=null) {
						...
						if ( wishlistUrl.search("ajaxcartxLogin.loadLoginPopup") != -1 ) {
							newWishlistUrl = wishlistUrl.replace("javascript:ajaxcartxLogin.loadLoginPopup('","").replace("', false);","");
							wishlistElements[i].setAttribute("href","javascript:ajaxcartx.addToWishlist('"+ newWishlistUrl +"',false);");
						} 	
					} 	
				}				
			}			
		...
		if (response.close_popup && jQueryAC('#'+response.close_popup+'-popup-container').css("left")=='0px') {	
			ajaxcartxTools.hidePopup(response.close_popup, false);  
		} 

		if (response.popup) {
			setTimeout(function() {
				ajaxcartxTools.showPopup(response.popup, false);
			},300);
		}
	},	
	...
	loadLoginPopup: function(url, popup) {
		this.addToWishlistUrl = url.replace("javascript:ajaxcartxLogin.loadLoginPopup('","").replace("', false);","");
		...
		var productId = ajaxcartxTools.getProductIdFromUrl(url,'product');
		if (popup=='success' && jQueryAC('#ac-cart-button'+productId).length>0) {
			ajaxcartx.addToCartButton = jQueryAC('#ac-cart-button'+productId);
		}
		
		ajaxcartxTools.setLoadWaiting('ajaxcartx-loading', true);		
		jQueryAC(this).ajaxSubmit({ 
		    url: ajaxcartxLogin.loginUrl,
		    type: 'post',
		    data: formData,
		    dataType: 'text',
		    success: function(response) {
		        ajaxcartxLogin.save(response);
		    },		
		    error: function() {
		        ajaxcartxLogin.failure();
		    }
		}); 
	},	
	...
	postLogin: function() {
        ...
        if (dataForm.validator.validate()){
			...
			if (window.location.href.indexOf('catalog/product_compare/index/')!=-1 && jQueryAC('#'+ajaxcartx.comparePopup) && jQueryAC('#login-form-validate #is_compare_popup').length==0) {
			...
			if (ajaxcartx.updateWindow.location.href.indexOf('checkout/cart')!=-1 && jQueryAC('#login-form-validate #is_cart_page').length==0) {
	...
	updateLoginLink: function() {   	
    	...
    	jQueryAC('#ac-links a', ajaxcartx.updateWindow.document).each(function() {
...
Ajaxcartx.prototype = {
	initialize: function(initConfig){
		...
		this.lastCartSidebar = (jQueryAC('#'+this.cartSidebar+'2').length>0) ? 2 : this.lastCartSidebar;
	...
	updateAjaxCartBlocks: function(alreadyLoaded){	
		...
		if (jQueryAC('meta[name="viewport"]').length > 0) {
			ajaxcartx.initialViewport = jQueryAC('meta[name="viewport"]').attr('content');
		}

		//set the maxWidth of client; used in jquery-ui.js for scrolling
    	ajaxcartx.maxWidth = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
		...
		if (jQueryAC('#product_addtocart_form').length > 0) {
			...
			var productId = ajaxcartxTools.getProductIdFromUrl(formAction,'product');
			    
			//add increase/decrease buttons to qty input
			if (jQueryAC('#qty').length > 0) {	
			    ajaxcartx.addQtyBoxHtml('ac-product-view', jQueryAC('#qty'), productId);	
			    ...
			    for ( var j = 0; j<inputElements.length; j++ ) {
			    	if (inputElements[j].name != null && inputElements[j].name != '') {
			    		...
			    		if ( qtyName.search("super_group") != -1 ) {			
			    			...
			    			ajaxcartx.addQtyBoxHtml('ac-product-view-grouped', jQueryAC(inputElements[j]), productId);												
			    		}
			    	}
			    }		
			}
		        
		    ajaxcartx.updateWindow.dispatchLiveUpdates('product', false);
		}
		
		//UPDATE CART SIDEBAR	
		if (jQueryAC('.'+ajaxcartx.cartSidebar, ajaxcartx.updateWindow.document).length > 0) {	
			for (var i=0;i<=ajaxcartx.lastCartSidebar;i++) {
				//add tooltip
				if (i==ajaxcartx.lastCartSidebar && ajaxcartx.enableTooltip != 0 && (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 2) && droppableSidebars['cart'] && !jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar + ' span.tooltip-sidebar', ajaxcartx.updateWindow.document).length > 0 && !jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar).hasClass('ac-header')){		
					jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar + ' div', ajaxcartx.updateWindow.document).first().append('<span class="tooltip-sidebar">' + Translator.translate('Buy Me!').stripTags() + '</span>');
				}
				
				//make the cart sidebar a droppable area	
				if(i==ajaxcartx.lastCartSidebar && (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 2) && droppableSidebars['cart'] && !jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar).hasClass('ac-header') && !ajaxcartx.isMobile && !ajaxcartx.isTablet) {
				    ajaxcartx.makeCartDroppableArea();		
				}
				
				//add update/empty sidebar cart buttons to cart sidebar, on first load
			    if(jQueryAC('#'+ajaxcartx.cartSidebar + i).length>0 && (jQueryAC('#'+ajaxcartx.cartSidebar+i, ajaxcartx.updateWindow.document).html().indexOf('<ac-qty>') != -1 || jQueryAC('#'+ajaxcartx.cartSidebar+i, ajaxcartx.updateWindow.document).html().indexOf('&lt;ac-qty&gt;') != -1) && !jQueryAC('#'+ajaxcartx.cartSidebar + i).hasClass('ac-header')){
			    	var actionsContainer = '<div class="actions" id="ajaxcartx-actions" style="border-bottom:none;">';
			    	actionsContainer += '<button onclick="ajaxcartx.emptyCartConfirmation();" class="button left" title="' + Translator.translate('Empty Cart').stripTags() + '" type="button"><span><span>' + Translator.translate('Empty Cart').stripTags() + '</span></span></button>';
			    	if (ajaxcartx.sidebarQty == 1){
			    		actionsContainer += '<button onclick="ajaxcartx.updateQty(\'\',false,\'' + ajaxcartx.cartSidebar + i + '\')" class="button" title="' + Translator.translate('Update Cart').stripTags() + '" type="button"><span><span>' + Translator.translate('Update Cart').stripTags() + '</span></span></button>';
			    	}		
			    	actionsContainer += '</div>';
			    	
			    	jQueryAC('#'+ajaxcartx.cartSidebar + i +' div').first().append(actionsContainer);
			    }	

				//add qty inputs + increase/decrease buttons
				if (jQueryAC('#'+ajaxcartx.comparePopup).length==0) {
					if (jQueryAC('#'+ajaxcartx.cartSidebar+i, ajaxcartx.updateWindow.document).html().indexOf('"<ac-qty>') != -1 || jQueryAC('#'+ajaxcartx.cartSidebar+i, ajaxcartx.updateWindow.document).html().indexOf('"&lt;ac-qty&gt;') != -1) {
						//if input exists
						jQueryAC('#'+ajaxcartx.cartSidebar+i+' input', ajaxcartx.updateWindow.document).each(function() {
							var input = jQueryAC(this);
							if (input.val().indexOf('<ac-qty>') != -1 || input.val().indexOf('&lt;ac-qty&gt;') != -1) {	
								var value = input.val().replace('<ac-qty>','').replace('</ac-qty>','').replace('&lt;ac-qty&gt;','').replace('&lt;/ac-qty&gt;','');
						    	...
						    	ajaxcartx.addQtyBoxHtml('ac-cart-sidebar', jQueryAC(this), 'item'+itemId);	
							}
						});
					} else if (jQueryAC('#'+ajaxcartx.cartSidebar+i, ajaxcartx.updateWindow.document).html().indexOf('<ac-qty>') != -1) {
						//if input doesn't exists
						if (ajaxcartx.sidebarQty==1) {
							jQueryAC('#'+ajaxcartx.cartSidebar+i+' ac-qty', ajaxcartx.updateWindow.document).each(function() {
								var qtyTag = jQueryAC(this);
								var value = qtyTag.text();
								var itemId = value.split('-')[0];
								var qty = value.split('-')[1];
								if (jQueryAC('#'+ajaxcartx.cartSidebar+i+' #sidebar-qty-'+itemId).length==0) {
									qtyTag.html('<input id="sidebar-qty-'+itemId+'" class="input-text qty" name="cart['+itemId+'][qty]" value="'+qty+'" />');
									qtyTag.contents().unwrap();

								    ajaxcartx.addQtyBoxHtml('ac-cart-sidebar', jQueryAC('#'+ajaxcartx.cartSidebar+i+' #sidebar-qty-'+itemId, ajaxcartx.updateWindow.document), 'item'+itemId);	
								}
							});
						}				
					}
			    ...
			    ajaxcartx.updateWindow.dispatchLiveUpdates('cart_sidebar', false);
			}
		}
		
		//UPDATE CART PAGE
		jQueryAC('#'+ajaxcartx.cartPage+' input', ajaxcartx.updateWindow.document).each(function() {
			...
			if (input.attr('name') != null && input.attr('name') != '' && input.attr('name').indexOf('cart') != -1 && input.attr('name').indexOf('[qty]') != -1) {	
			    ...
			    ajaxcartx.addQtyBoxHtml('ac-cart-page', jQueryAC(this), 'item'+itemId);	
			}
		        
		    ajaxcartx.updateWindow.dispatchLiveUpdates('cart', false);
		});
		
		//UPDATE WISHLIST SIDEBAR
		if (jQueryAC('.'+ajaxcartx.wishlistSidebar, ajaxcartx.updateWindow.document).length > 0) {	
			for (var i=0;i<=ajaxcartx.lastWishlistSidebar;i++) {
				//add tooltip
				if (i==ajaxcartx.lastWishlistSidebar && ajaxcartx.enableTooltip != 0 && (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 2) && droppableSidebars['wishlist'] && !jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar +' span.tooltip-sidebar', ajaxcartx.updateWindow.document).length > 0 && !jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar).hasClass('ac-header')){		
					jQueryAC('#'+ajaxcartx.wishlistSidebar+ ajaxcartx.lastWishlistSidebar +' div', ajaxcartx.updateWindow.document).first().append('<span class="tooltip-sidebar">' + Translator.translate('Wish Me!').stripTags() + '</span>');
				}
				
				//make the wishlist sidebar a droppable area
				if(i==ajaxcartx.lastWishlistSidebar && (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 2) && droppableSidebars['wishlist'] && !jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar).hasClass('ac-header') && !ajaxcartx.isMobile && !ajaxcartx.isTablet) {
				    ajaxcartx.makeWishlistDroppableArea();		
				}	
			        
			    ajaxcartx.updateWindow.dispatchLiveUpdates('wishlist_sidebar', false);	
			}
		}
		
		//UPDATE WISHLIST PAGE
		jQueryAC('#'+ajaxcartx.wishlistPage+' input', ajaxcartx.updateWindow.document).each(function() {
			//add increase/decrease buttons to qty inputs
			var input = jQueryAC(this);
			if (input.attr('name') != null && input.attr('name') != '' && input.attr('name').indexOf('qty[') != -1) {	
			    ...
			    ajaxcartx.addQtyBoxHtml('ac-wishlist-page', jQueryAC(this), 'witem'+itemId);	
			}
			
		    truncateOptions();
		        
		    ajaxcartx.updateWindow.dispatchLiveUpdates('wishlist', false);
		})
		
		//UPDATE COMPARE SIDEBAR
		if (jQueryAC('.'+ajaxcartx.compareSidebar, ajaxcartx.updateWindow.document).length > 0) {	
			for (var i=0;i<=ajaxcartx.lastCompareSidebar;i++) {
				//add tooltip
				if (i==ajaxcartx.lastCompareSidebar && ajaxcartx.enableTooltip != 0 && (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 2) && droppableSidebars['compare'] && !jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar + ' span.tooltip-sidebar', ajaxcartx.updateWindow.document).length > 0 && !jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar).hasClass('ac-header')){		
					jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar + ' div', ajaxcartx.updateWindow.document).first().append('<span class="tooltip-sidebar">' + Translator.translate('Compare Me!').stripTags() + '</span>');
				}
				
				//make the compare sidebar a droppable area
				if((ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 2) && droppableSidebars['compare'] && !jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar).hasClass('ac-header') && !ajaxcartx.isMobile && !ajaxcartx.isTablet) {
				    ajaxcartx.makeCompareDroppableArea();		
				}
			        
			    ajaxcartx.updateWindow.dispatchLiveUpdates('compare_sidebar', false);
			}
		}

		jQueryAC(document).ready(function(){ajaxcartxTools.runLivicons();});
	},		

	updateLinksButtons: function() {
		var delay = (window.opener != null && window.location.href.indexOf('catalog/product_compare/index/')!=-1 && jQueryAC('#'+ajaxcartx.comparePopup)>0) ? 20 : 0;
		//Change add to cart buttons	
		setTimeout(function() { ajaxcartx.updateButtons(ajaxcartx.updateWindow.document); }, delay);	
				    
		//Change cart/wishlist/compare links + add product id attribute to images, used for drag and drop
		setTimeout(function() { ajaxcartx.updateLinks(ajaxcartx.updateWindow.document); }, delay);	

		//update the parent window as well if actions are performed from the compare popup
		if (window.location.href.indexOf('catalog/product_compare/index/')!=-1 && jQueryAC('#'+ajaxcartx.comparePopup).length>0) {
			ajaxcartx.updateLinks(window.document);	
			ajaxcartx.updateButtons(window.document);	
		}
	},
	
	//change the normal link with the ajaxcartx equivalent link
	updateLinks: function(updateWindow) {
		jQueryAC('a', updateWindow).each(function() {
			...
			if (a.onclick && a.onclick != null && a.onclick != '' && a.getAttributeNode('onclick')) {
		    	...
		    	if ( onClick.indexOf("checkout/cart/add") != -1 && onClick.indexOf("/product/") != -1 ) {
		    		if ( onClick.indexOf("setPLocation") != -1 ) {
			    		var url = onClick.replace("setPLocation('","").replace("', false)","").replace("', true)","").replace("checkout/cart/add","ajaxcartx/index/init");
			    	} else {
			    		var url = onClick.replace("setLocation('","").replace("')","").replace("checkout/cart/add","ajaxcartx/index/init");
			    	}
				    var productId = ajaxcartxTools.getProductIdFromUrl(url,'product');
		    		a.setAttribute("onclick","ajaxcartx.initAjaxcartx('"+ url +"', this, false);");
		    		
		    		//ie7 fix
					if (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion == '7.0'){
		    	...
		    	ajaxcartx.updateWindow.dispatchLinkUpdates(a, onClick);	 
	    	}
			...
			if (a.href != null && a.href != '') {	
			    ...
			    //Change cart links
			    if ( link.indexOf("checkout/cart/add") != -1 && link.indexOf("/product/") != -1 ) {
		    		var newLink = link.replace("checkout/cart/add","ajaxcartx/index/init");
				    var productId = ajaxcartxTools.getProductIdFromUrl(newLink,'product');
		    		a.setAttribute("href","javascript:ajaxcartx.initAjaxcartx('"+ newLink +"', this, false);");
			    ...
			    if ( link.indexOf("checkout/cart/delete") != -1 || link.indexOf("checkout/cart/ajaxDelete") != -1 ) {
					var itemId = ajaxcartxTools.getProductIdFromUrl(link,'id');
					// a.setAttribute("href","javascript:ajaxcartx.removeCartItem("+itemId+");");
			        a.setAttribute("onClick","ajaxcartx.removeCartItem('"+itemId+"');");
				...
				if ( link.indexOf("checkout/cart/configure/") != -1 ) {
			        newLink = link.replace("checkout/cart/configure/","ajaxcartx/cart/configure/");
			        if (ajaxcartx.isSecure && newLink.indexOf("https") == -1) {
			        	newLink = newLink.replace("http","https");
			        }
			        a.setAttribute("onClick","ajaxcartx.configureCartItem('"+newLink+"');");
			        a.setAttribute("href","javascript:void(0);");
			    } 		
			
			    //Change compare links
			    if ( link.indexOf("catalog/product_compare/add/") != -1 ) {
				    var productId = ajaxcartxTools.getProductIdFromUrl(link,'product');
			        newLink = link.replace("catalog/product_compare/add/","ajaxcartx/product_compare/add/");
			        if (ajaxcartx.isSecure && newLink.indexOf("https") == -1) {
			        	newLink = newLink.replace("http","https");
			        }
			        a.setAttribute("href","javascript:ajaxcartx.addToCompare('"+ newLink +"', false);");
			    ...
			    if ( link.indexOf("catalog/product_compare/clear/") != -1 ) {
			        newLink = link.replace("catalog/product_compare/clear/","ajaxcartx/product_compare/clear/");
			        a.setAttribute("href","javascript:ajaxcartx.removeCompareItems('"+ newLink +"');");
			    } 	
			    if ( link.indexOf("catalog/product_compare/remove/") != -1 ) {
			        newLink = link.replace("catalog/product_compare/remove/","ajaxcartx/product_compare/remove/");
			        a.setAttribute("href","javascript:ajaxcartx.removeCompareItem('"+newLink+"');");
			    } 	
				...
				if (a.onclick && a.onclick != null && a.onclick != '' && a.getAttributeNode('onclick')) {
					...
					if (onClick.indexOf("catalog/product_compare/remove/") != -1) {
				        newLink = onClick.replace("catalog/product_compare/remove/","ajaxcartx/product_compare/remove/").replace("removeItem('","").replace("');","");
						a.setAttribute("onclick","ajaxcartx.removeCompareItem('"+newLink+"');");
					...
					if ( onClick.indexOf("catalog/product_compare/add/") != -1 ) {
					    var productId = ajaxcartxTools.getProductIdFromUrl(onClick,'product');
				        newLink = onClick.replace("catalog/product_compare/add/","ajaxcartx/product_compare/add/").replace("setLocation('","").replace("')","");;
				        if (ajaxcartx.isSecure && newLink.indexOf("https") == -1) {
				        	newLink = newLink.replace("http","https");
				        }
				        a.setAttribute("href","javascript:ajaxcartx.addToCompare('"+ newLink +"', false);");
			    ...
			    if(ajaxcartxLogin.isLoggedIn){
			        if ( link.indexOf("wishlist/index/add/") != -1 ) {
				    	var productId = ajaxcartxTools.getProductIdFromUrl(link,'product');
			            newLink = link.replace("wishlist/index/add/","ajaxcartx/wishlist/add/");
			            a.setAttribute("href","javascript:ajaxcartx.addToWishlist('"+ newLink +"',false);");
			        	...
			        	var onClick = a.getAttributeNode("onclick").nodeValue.toString();
				        var productId = ajaxcartxTools.getProductIdFromUrl(onClick,'product');
			            newLink = onClick.replace("setPLocation('","").replace("', false)","").replace("', true)","").replace("wishlist/index/add/","ajaxcartx/wishlist/add/");
			            a.setAttribute("onclick","ajaxcartx.addToWishlist('"+ newLink +"',false);");
			        }	
			        if ( link.indexOf("wishlist/index/cart") != -1 ) {
			            newLink = link.replace("wishlist/index/cart/","ajaxcartx/wishlist/cart/");
			            a.setAttribute("href","javascript:ajaxcartx.addWishlistItemToCart('"+newLink+"', false);");
			        } 
			        if ( link.indexOf("wishlist/index/configure") != -1 ) {
			            newLink = link.replace("wishlist/index/configure/","ajaxcartx/wishlist/configure/");
			            a.setAttribute("href","javascript:ajaxcartx.configureWishlistItem('"+newLink+"');");
			        } 
			        if ( link.indexOf("wishlist/index/remove") != -1 ) {
			            newLink = link.replace("wishlist/index/remove/","ajaxcartx/wishlist/remove/");
			            a.setAttribute("href","javascript:ajaxcartx.removeWishlistItem('"+newLink+"');");
			        } 	
			        if ( link.indexOf("ajaxcartxLogin.loadLoginPopup") != -1 ) {
						newLink = link.replace("javascript:ajaxcartxLogin.loadLoginPopup('","").replace("', false);","");
						a.setAttribute("href","javascript:ajaxcartx.addToWishlist('"+ newLink +"',false);");
					} 
					//update move to wishlist link from shopping cart page
					if ( link.indexOf("wishlist/index/fromcart/") != -1 ) {
				        newLink = link.replace("wishlist/index/fromcart/","ajaxcartx/wishlist/fromcart/");
				        a.setAttribute("href","javascript:ajaxcartx.moveToWishlist('"+newLink+"');");
				    } 	
			    } else {
			        if ( link.indexOf("wishlist/index/add/") != -1 ) {
				    	var productId = ajaxcartxTools.getProductIdFromUrl(link,'product');
			            newLink = link.replace("wishlist/index/add/","ajaxcartx/wishlist/add/");
			            a.setAttribute("href","javascript:ajaxcartxLogin.loadLoginPopup('"+newLink+"', false);");
			            //remove onclick from compare popup
			            a.setAttribute("onclick","");
	...
	//change the normal button with the ajaxcartx equivalent button
		...
		jQueryAC('button', updateWindow).each(function() {
			...
			if (button.onclick != null && button.onclick != '') {
		    	...
		    	if ( onClick.indexOf("checkout/cart/add") != -1 && onClick.indexOf("/product/") != -1 ) {
			    	...
			    	if (ajaxcartx.isSecure && url.indexOf("https") == -1) {
			        	url = url.replace("http","https");
			        }

				    var productId = ajaxcartxTools.getProductIdFromUrl(url,'product');
		    		button.setAttribute("onclick","ajaxcartx.initAjaxcartx('"+ url +"', this, false);");
					...
					button.setAttribute("onclick","ajaxcartx.initAjaxcartx(false, this, false);");
					...
					jQueryAC(button).click(function( event ) {
					    ...
					    ajaxcartx.updateWishlistItems();
					});
				} else if (onClick.indexOf("addWItemToCart(") != -1) {
		    		var itemId = onClick.replace("addWItemToCart(","").replace(");","").replace(")","");
			    	button.setAttribute("onclick","ajaxcartx.initAddWishlistItemToCart("+itemId+");");
			    	...
			    	button.setAttribute("onclick","ajaxcartx.addAllWishlistItemsToCart();");
	...
	updateProductLists: function(alreadyLoaded) {
		...
		jQueryAC('.' + ajaxcartx.categoryList + ' li').each(function() {				
			var li = this;
			...
			for ( var i = 0; i<buttons.length; i++ ) {
			    if (buttons[i].onclick != null && buttons[i].onclick != '') {
			    	...
			    	if ( onclickCartAction.indexOf("checkout/cart/add") != -1 || onclickCartAction.indexOf("ajaxcartx/index/init") != -1) {
			    		isSaleable = true;
			    		
			    		//add qty inputs, increase/decrease buttons
			    		var productId = ajaxcartxTools.getProductIdFromUrl(onclickCartAction,'product');
			    		ajaxcartx.addQtyBoxHtml('ac-product-list', buttons[i], productId);
			    	}
			    }
			}
				...
				for ( var i = 0; i<links.length; i++ ) {
				    if (links[i].onclick != null && links[i].onclick != '') {
				    	...
				    	if ( onclickCartAction.indexOf("checkout/cart/add") != -1 || onclickCartAction.indexOf("ajaxcartx/index/init") != -1) {
				    		...
				    		//add qty inputs, increase/decrease buttons
				    		var productId = ajaxcartxTools.getProductIdFromUrl(onclickCartAction,'product');
				    		ajaxcartx.addQtyBoxHtml('ac-product-list', links[i], productId);
				    	}
				    }
				}
			}
			...
			if (!isSaleable && (ajaxcartx.dragdropCategory != 0 || ajaxcartx.jump == 1)) {
			    var links = li.getElementsByTagName('a');					
			    for ( var i = 0; i<links.length; i++ ) {
			    	if (links[i].href != null && links[i].href != '') {
			    		link = links[i].href.toString();
			    	    if (link.indexOf("ajaxcartx/wishlist/add/") != -1 || link.indexOf("wishlist/index/add/") != -1 || link.indexOf("ajaxcartx/product_compare/add/") != -1 || link.indexOf("catalog/product_compare/add/") != -1) {
			    	    	//add productId attribute to image for drag and drop	
			    	    	var productId = ajaxcartxTools.getProductIdFromUrl(links[i].href,'product');
			    		    break;
			    	    }
			        }
			    }
			...
			//make item draggable
			if (alreadyLoaded) {
				ajaxcartx.makeItemDraggable(li, isSaleable, productId);
			} else {
	...
	updateSections: function(response){
		...
		if (response.update_section.html_layout_messages) {
		    setTimeout(function () { jQueryAC('#ajaxcartx-layout-messages').html(response.update_section.html_layout_messages); }, ajaxcartx.successPopupDelay);
		}	

		//update additional content in success popup
		if (response.update_section.html_additional_content) {
		    setTimeout(function () { jQueryAC('#ajaxcartx-custom-block').html(response.update_section.html_additional_content); ajaxcartx.updateProductLists(); }, ajaxcartx.successPopupDelay);
		}	

		//update product images in success popup
		if (response.update_section.html_product_images) {
		    setTimeout(function () { jQueryAC('#ajaxcartx-product-images').html(response.update_section.html_product_images); }, ajaxcartx.successPopupDelay);
		}	
		
		if (response.update_section.html_options_layout_messages) {
		    jQueryAC('#ajaxcartx-options-layout-messages').html(response.update_section.html_options_layout_messages);
		}
		
		//Update siderbar blocks
		if (!ajaxcartxLogin.loginPostResponse) {		
			//reinitialize the ajaxcartx js if an item is added/updated while on the shopping cart page
			if (response.update_section.html_ajaxcartx_js) {
				ajaxcartxTools.updateSection(response.update_section.html_ajaxcartx_js,'ajaxcartx-qty-js');
			}	
			
			//update options popup
			if (response.update_section.html_options) {
				ajaxcartxTools.updateSection(response.update_section.html_options,'ajaxcartx-options');
				this.updateOptionsPopupAddToCartButton(response);
			}
		
			//preload cart/wishlist sidebar images
			if (response.update_section.html_cart) {
				ajaxcartxTools.preloadImages(response.update_section.html_cart);
			}
			if (response.update_section.html_wishlist) {
				ajaxcartxTools.preloadImages(response.update_section.html_wishlist);
			}
			
			setTimeout(function() {
				ajaxcartx.updateCart(response);
				ajaxcartx.updateCartPage(response);			
				ajaxcartx.updateWishlist(response);
				ajaxcartx.updateWishlistPage(response);	
				ajaxcartx.updateCompare(response);				
				ajaxcartx.updateComparePopup(response);	
				ajaxcartx.updateLinksButtons();
					
				//this function is used to launch specific events for custom themes
				ajaxcartx.updateWindow.dispatchBlockUpdates(response);			
			},300);
		} else {	
			//close login popup
			ajaxcartxTools.hidePopup('ajaxcartx-login', false);		
			
			//preload cart/wishlist sidebar images			
			if (ajaxcartxLogin.loginPostResponse.update_section.html_cart) {
				ajaxcartxTools.preloadImages(ajaxcartxLogin.loginPostResponse.update_section.html_cart);
			}
			if (ajaxcartxLogin.loginPostResponse.update_section.html_wishlist) {
				ajaxcartxTools.preloadImages(ajaxcartxLogin.loginPostResponse.update_section.html_wishlist);
			}
			...
			setTimeout(
				function(){		
	    		    ...
	    		    //update login link
	    			ajaxcartxLogin.updateLoginLink();	 
	    				
					//update welcome message
					if (ajaxcartxLogin.loginPostResponse.update_section.welcome && ajaxcartx.updateWindow.document.getElementById('ac-welcome-message')) {
						ajaxcartx.updateWindow.document.getElementById('ac-welcome-message').innerHTML = ajaxcartxLogin.loginPostResponse.update_section.welcome;
					}	
					
					//reinitialize the ajaxcartx js
					if (ajaxcartxLogin.loginPostResponse.update_section.html_ajaxcartx_js) {
						ajaxcartxTools.updateSection(ajaxcartxLogin.loginPostResponse.update_section.html_ajaxcartx_js,'ajaxcartx-qty-js');
					}		
					
					ajaxcartx.updateCart(ajaxcartxLogin.loginPostResponse);
					ajaxcartx.updateCartPage(ajaxcartxLogin.loginPostResponse);	
					ajaxcartx.updateWishlist(response);		
					ajaxcartx.updateWishlistPage(response);
					ajaxcartx.updateCompare(ajaxcartxLogin.loginPostResponse);
					ajaxcartx.updateComparePopup(ajaxcartxLogin.loginPostResponse);	
					ajaxcartx.updateLinksButtons();
					
					//this function is used to launch specific events for custom themes
					ajaxcartx.updateWindow.dispatchBlockUpdates(ajaxcartxLogin.loginPostResponse);	
					
					//reset values
					ajaxcartxLogin.loginPostResponse = false;	
					ajaxcartxLogin.ajaxCartLoginRunning = false;
	...
	updateOptionsPopupAddToCartButton: function(response){
		var buttonElements = document.getElementById('ajaxcartx-options').getElementsByTagName('button');
		...
		for ( var i = 0; i<buttonElements.length; i++ ) {
			if (buttonElements[i].onclick != null && buttonElements[i].onclick != '') {
				...
				if ( onClick === 'productAddToCartForm.submit(this)' || onClick === 'productAddToCartForm.submit()' ) {
					if (link.indexOf('/wishlist/index/cart/')!=-1) {						
		            	onClick = "ajaxcartx.addWishlistItemToCart('" + link.replace("wishlist/index/cart/","ajaxcartx/wishlist/cart/") + "skip_popup/1/', this);";
		            } else if (link.indexOf('/wishlist/index/updateItemOptions/')!=-1) {
		            	onClick = "ajaxcartx.updateWishlistItem('" + link.replace("wishlist/index/updateItemOptions/","ajaxcartx/wishlist/updateItemOptions/") + "skip_popup/1/', this);";
		            	...
		            	onClick = "ajaxcartx.updateCartItem('" + link.replace("checkout/cart/updateItemOptions/","ajaxcartx/cart/updateItemOptions/") + "close_popup/1/', this);";
		            	if (ajaxcartx.isSecure && onClick.indexOf("https") == -1) {
				        	onClick = onClick.replace("http","https");
				        }
				        jQueryAC(buttonElements[i]).html("<span><span>" + Translator.translate('Update Cart') + "</span></span>");
				    } else {
		            	onClick = "ajaxcartx.initAjaxcartx('"+ link +"', this, 'options');";
		            }
		...
		if (jQueryAC('#options_addtocart_form #qty').length>0){	
			ajaxcartx.addQtyBoxHtml('ac-options-popup', jQueryAC('#options_addtocart_form #qty'), response.product_id);
			if (response.qty) {		
			...
			var inputElements = document.getElementById('ajaxcartx-options').getElementsByTagName('input');
			for ( var j = 0; j<inputElements.length; j++ ) {
				if (inputElements[j].name != null && inputElements[j].name != '') {
					var qtyName = inputElements[j].getAttributeNode("name").nodeValue.toString();						
					if ( qtyName.search("super_group") != -1 ) {			
						var productId = qtyName.replace("super_group[","").replace("]","");
						ajaxcartx.addQtyBoxHtml('ac-options-popup-grouped', jQueryAC(inputElements[j]), productId);	
						if (response.qty) {		
							if (isNaN(response.qty)){
	...
	addQtyBoxHtml: function(section, element, productId) {
		...
		if (section=='ac-product-list') {
			...
			addInput = ajaxcartx.categoryQty;
			if (addInput==1) {
				addInputButtons = ajaxcartx.categoryQtyButtons;
			} 
		} else if (section=='ac-product-view') {
			var name = "qty";
			addInputButtons = ajaxcartx.productQtyButtons;
			...
			addInputButtons = ajaxcartx.sidebarQtyButtons;
		} else if (section=='ac-cart-page') {
			var itemId = element.attr('id').replace(/\D/g, '' );
			var name = "cart-qty-"+itemId;
			addInputButtons = ajaxcartx.cartPageQtyButtons;
		} else if (section=='ac-wishlist-page') {
			var itemId = element.attr('id').replace(/\D/g, '' );
			var name = "wishlist-qty-"+itemId;
			addInputButtons = ajaxcartx.wishlistPageQtyButtons;
		} else if (section=='ac-options-popup') {
			addInputButtons = ajaxcartx.popupQtyButtons;
			var name = "popup-qty";
			element.attr('id',name);
		...
		if (addInput == 1 && addInputButtons == 1){
		    qtyBoxHtml = "<span class='ajaxcartx-qty'><input name='"+name+"' id='"+name+"' type='text' value='"+ productMinMax[productId]['min'] +"' class='input-text qty'><span class='qty-control-box'><button type='button' class='increase' href='javascript:void(0)' onclick='ajaxcartx.qtyUp(" + productId + ",\""+name+"\", this);'>+</button><button type='button' class='decrease' href='javascript:void(0)' onclick='ajaxcartx.qtyDown(" + productId + ",\""+name+"\", this);'>-</button></span></span>";
		} else if (addInput == 1) {
		    qtyBoxHtml = "<span class='ajaxcartx-qty'><input name='"+name+"' id='"+name+"' type='text' value='"+ productMinMax[productId]['min'] +"' class='input-text qty'></span>";
		} else if (addInputButtons == 1) {
		    qtyBoxHtml = "<span class='qty-control-box'><button type='button' class='increase' href='javascript:void(0)' onclick='ajaxcartx.qtyUp(\"" + productId + "\",\""+name+"\",this);'>+</button><button type='button' class='decrease' href='javascript:void(0)' onclick='ajaxcartx.qtyDown(\"" + productId + "\",\""+name+"\",this);'>-</button></span>";
		}		
		...
		} else if ((section=='ac-cart-page' || section=='ac-cart-sidebar' || section=='ac-wishlist-page') && qtyBoxHtml!='') {
			element.wrap('<span class="ajaxcartx-qty"></span>');
			element.after(qtyBoxHtml);
		} else if (element.parent().prop('outerHTML').indexOf('ajaxcartx-qty') == -1 && qtyBoxHtml!='') {
			element.prop('outerHTML', '<span class="ajaxcartx-qty">'+ element.prop('outerHTML') + qtyBoxHtml + '</span>');
		} 		
	...
	makeItemDraggable: function(li, isSaleable, productId) { 
		if ((ajaxcartx.dragdropCategory != 0 || ajaxcartx.jump == 1) && !jQueryAC(li).hasClass('disable-dragdrop') && !ajaxcartx.isMobile && !ajaxcartx.isTablet){
			var imgTotal = 0,
				lastImage = '',
				imgCount = 0;
			...
			jQueryAC(li).find('img').each(function() {
	            ...
	            if (src != '' && src != null && src.indexOf('media/catalog/product/')!=-1 && jQueryAC('.draggable-bkg', li).length == 0){
	            	...
	            	if (imgCount==imgTotal) {						
				    	...
				    	if (ajaxcartx.dragdropCategory != 0) {
					    	var dataSize = 60;
					    	if (width>140) {
					    		var dataSize = 65;
					    	} else if (width>180) {
					        ...
					        jQueryAC('.ac-drag-livicon').addLivicon({
							    name : 'drag',
							    size : dataSize,
							    color : ajaxcartx.productImageIconColor,
							    animate : false
						    });
						    ...
						    imgContainer.hover(function(){
						    	...
						    	if (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion <= 8){
							    	jQueryAC(this, this).find('.draggable-bkg').css({'display' : 'block'});				    		
						    	} else {
						    		running = true;
									jQueryAC(this, this).find('.draggable-bkg').css({'box-shadow': '0px 0px 0px '+spread+'px rgba('+ajaxcartx.boxShadowColor+',0.8) inset'});
								}
						    }, function() {
						    	...
						    	if (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion <= 8){
							    	...
							    	if ((draggableBkg.css('box-shadow')).indexOf(spread+'px')!=-1) {
							    		draggableBkg.css({'box-shadow': '0px 0px 0px '+spread+'px rgba('+ajaxcartx.boxShadowColor+',0.8) inset'});
							    		setTimeout(function() {
							    			if (!running) {
							    				draggableBkg.addClass('no-transition');
							    				draggableBkg.css({'box-shadow': '0px 0px 0px -1px rgba('+ajaxcartx.boxShadowColor+',0.8) inset'});	
						    ...
						    imgContainer.draggable({ 
						    	...
						    	start: function(event, ui) {
						    		ajaxcartx.dragging = true;

							    	//set the viewport meta tag for mobile devices; required for drag and drop
									if (ajaxcartx.isMobile || ajaxcartx.isTablet) {		
										ajaxcartxTools.setViewportMeta();
									}

						    		//activate tooltip
						    		if (ajaxcartx.enableTooltip){	
						    			if (!isSaleable) {
						    				jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar +' span.tooltip-sidebar').remove();
						    			}
						    		...
						    		ajaxcartx.initFloaters(imgContainer.attr('product'));
							    	...
							    	if (ajaxcartx.dropEffect != 'noeffect') {
										img.css({'border-radius' : '50%', 'box-shadow':'0 2px 0px 1px rgba(0, 0, 0, 0.1)'});
										imgContainer.find('.draggable-bkg').css({'border-radius' : '50%'});
									}
							    	
							    	//ajaxcartx.dragContainer is used in jquert-ui.js
						    		ajaxcartx.dragContainer = imgContainer;			    		
						    	},
						    	stop: function(event, ui) {		
									if (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion <= 9){
										if (ajaxcartx.itemDropped) {	
											imgContainer.fadeIn();
										} else {
									...
									if (ajaxcartx.isMobile || ajaxcartx.isTablet) {	
										if (ajaxcartx.initialViewport) {
							    			jQueryAC('meta[name="viewport"]').attr('content', ajaxcartx.initialViewport);
							    		} else {
							    			jQueryAC('meta[name="viewport"]').attr('content', '');
							    		}
							    	}

									ajaxcartx.itemDropped = false;
						    	}
						    })
						} else {
	...
	makeCartDroppableArea: function() { 
	    jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar).droppable({
	    	accept: function() { return true; },
	    	over: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['cart']){
	    			jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar + ' div').first().addClass('draggable-over');
	    		}
	    	},
	    	out: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['cart']){
	    			jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar + ' div').first().removeClass('draggable-over');
	    		}
	    	},
	    	drop: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['cart']){
		    		ajaxcartx.itemDropped = true;
		    		ajaxcartx.initAjaxcartx(dragDropProducts[ui.draggable.attr('product')]['cart'], ui.draggable, false);		 
	    			jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar + ' div').first().removeClass('draggable-over');
	    		}
	    	}
	    })		
	...
	makeCompareDroppableArea: function() { 
	    jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar).droppable({
	    	accept: function() { return true; },
	    	over: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['compare']){
	    			jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar + ' div').first().addClass('draggable-over');
	    		}
	    	},
	    	out: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['compare']){
		    		jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar + ' div').first().removeClass('draggable-over');
		    	}
	    	},
	    	drop: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['compare']){
	    			ajaxcartx.itemDropped = true;
					ajaxcartx.addToCompare(dragDropProducts[ui.draggable.attr('product')]['compare'], false);
					jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar + ' div').first().removeClass('draggable-over');
				}
	    	}
	    })
	...
	makeWishlistDroppableArea: function() { 
	    jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar, ajaxcartx.updateWindow.document).droppable({
	    	accept: function() { return true; },
	    	over: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['wishlist']){
	    			jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar +' div').first().addClass('draggable-over');
	    		}
	    	},
	    	out: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['wishlist']){
		    		jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar +' div').first().removeClass('draggable-over');
		    	}
	    	},
	    	drop: function( event, ui ) {
	    		if(ui.draggable.attr('product') && dragDropProducts[ui.draggable.attr('product')]['wishlist']){
		    		ajaxcartx.itemDropped = true;
					ajaxcartx.addToWishlist(dragDropProducts[ui.draggable.attr('product')]['wishlist'],false);
					jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar +' div').first().removeClass('draggable-over');
				}
	    	}
	    })
	...
	initFloaters: function(productId) {
		if (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 3) {
			this.floaterTotal = 0;
			this.displayCartFloater = this.displayFloater(jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar), productId, 'cart');
			this.displayCompareFloater = this.displayFloater(jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar), productId, 'compare');
			this.displayWishlistFloater = this.displayFloater(jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar), productId, 'wishlist');

		    if (this.displayCartFloater) {
		        this.cartFloater = jQueryAC('#cart-floater'+productId);
		        
			    //remove any initial classes
		    	this.cartFloater.removeClass('animated animate-mouse-over bounce-'+ajaxcartx.cartFloaterPosition+' ac-floater-'+ajaxcartx.cartFloaterPosition+' return-'+ajaxcartx.cartFloaterPosition); 
		        ...
		        setTimeout(function() { if (ajaxcartx.dragging) { cartFloater.addClass('ac-floater-'+cartFloaterPosition); cartFloater.removeClass('animated bounce-'+cartFloaterPosition); ajaxcartx.checkOverlap(); } }, 500);
		        
		        if (ajaxcartxTools.browserTypeIE && ajaxcartxTools.browserVersion < 10) {
		    ...
		    if (this.displayCompareFloater) {
			    ...
			    //remove any initial classes
		    	this.compareFloater.removeClass('animated animate-mouse-over bounce-'+ajaxcartx.compareFloaterPosition+' ac-floater-'+ajaxcartx.compareFloaterPosition+' return-'+ajaxcartx.compareFloaterPosition); 
		        ...
		        setTimeout(function() { if (ajaxcartx.dragging) { compareFloater.addClass('ac-floater-'+compareFloaterPosition); compareFloater.removeClass('animated bounce-'+compareFloaterPosition); ajaxcartx.checkOverlap(); } }, 500);
		    ...
		    if (this.displayWishlistFloater) {
			    ...
			    //remove any initial classes
		    	this.wishlistFloater.removeClass('animated animate-mouse-over bounce-'+ajaxcartx.wishlistFloaterPosition+' ac-floater-'+ajaxcartx.wishlistFloaterPosition+' return-'+ajaxcartx.wishlistFloaterPosition); 
		        ...
		        setTimeout(function() { if (ajaxcartx.dragging) { wishlistFloater.addClass('ac-floater-'+wishlistFloaterPosition); wishlistFloater.removeClass('animated bounce-'+wishlistFloaterPosition); ajaxcartx.checkOverlap(); } }, 500);
	...
	displayFloater: function(block, productId, type) {
		if (block.length > 0 && ajaxcartx.dragdropCategory != 3 && block.is(':visible') && !block.hasClass('ac-header')) {
	...
	getFloaterPosition: function(floaterId) {
		ajaxcartx.floaterTotal++;

		if (ajaxcartx.floaterTotal==1) {
			return 'top';
		} else if (ajaxcartx.floaterTotal==2) {
			if (!this.displayCartFloater && jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar).length>0) {
				if (jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar).offset().left>ajaxcartx[floaterId+'Floater'].offset().left) {
					return 'left';
				} else {
					return 'right';
				}
			} else if (!this.displayCompareFloater && jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar).length>0) {
				if (jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar).offset().left>ajaxcartx[floaterId+'Floater'].offset().left) {
					return 'left';
				} else {
					return 'right';
				}
			} else if (!this.displayWishlistFloater && jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar).length>0) {
				if (jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar).offset().left>ajaxcartx[floaterId+'Floater'].offset().left) {
					return 'left';
				} else {
					return 'right';
				}
		...
		} else if (ajaxcartx.floaterTotal==3) {
	...
	overlaps: function(a, b) {
    	...
    	if (b && b.css('opacity')==1 && (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 3)) {
	...
	checkOverlap: function() {
		if (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 3) {
			...
			if (this.overlaps(draggableHelper, ajaxcartx.cartFloater) && !this.floaterRunning.cart) {
				if (ajaxcartx.compareFloater) {
					ajaxcartx.compareFloater.removeClass('mouse-over');
				}
				if (ajaxcartx.wishlistFloater) {
					ajaxcartx.wishlistFloater.removeClass('mouse-over');		
				}

				ajaxcartx.cartFloater.addClass('animate-mouse-over mouse-over');		
				this.floaterRunning.cart = true;
				jQueryAC('#cart-livicon'+draggableHelper.attr('product')).trigger('mouseover');
			} else if (ajaxcartx.cartFloater && this.floaterRunning.cart && !this.overlaps(draggableHelper, ajaxcartx.cartFloater)) {
				this.floaterRunning.cart = false;
				this.floaterRunning.compare = false;
				this.floaterRunning.wishlist = false;
				ajaxcartx.cartFloater.removeClass('mouse-over');			
			}	

			if (this.overlaps(draggableHelper, ajaxcartx.compareFloater) && !this.floaterRunning.compare) {	
				if (ajaxcartx.cartFloater) {
					ajaxcartx.cartFloater.removeClass('mouse-over');		
				}
				if (ajaxcartx.wishlistFloater) {
					ajaxcartx.wishlistFloater.removeClass('mouse-over');		
				...
				ajaxcartx.compareFloater.addClass('animate-mouse-over mouse-over');	
				this.floaterRunning.compare = true;
				jQueryAC('#compare-livicon'+draggableHelper.attr('product')).trigger('mouseover');
			} else if (ajaxcartx.compareFloater && this.floaterRunning.compare && !this.overlaps(draggableHelper, ajaxcartx.compareFloater)) {
				this.floaterRunning.cart = false;
				this.floaterRunning.compare = false;
				this.floaterRunning.wishlist = false;
				ajaxcartx.compareFloater.removeClass('mouse-over');			
			...
			if (this.overlaps(draggableHelper, ajaxcartx.wishlistFloater) && !this.floaterRunning.wishlist) {	
				if (ajaxcartx.cartFloater) {
					ajaxcartx.cartFloater.removeClass('mouse-over');		
				}
				if (ajaxcartx.compareFloater) {
				...
				ajaxcartx.wishlistFloater.addClass('animate-mouse-over mouse-over');	
				this.floaterRunning.wishlist = true;
				jQueryAC('#wishlist-livicon'+draggableHelper.attr('product')).trigger('mouseover');
			} else if (ajaxcartx.wishlistFloater && this.floaterRunning.wishlist && !this.overlaps(draggableHelper, ajaxcartx.wishlistFloater)) {
				this.floaterRunning.wishlist = false;
				this.floaterRunning.cart = false;
				this.floaterRunning.compare = false;
				ajaxcartx.wishlistFloater.removeClass('mouse-over');			
	...
	save: function(transport){
		...
		//close options popup
		if (response.close_popup && jQueryAC('#'+response.close_popup+'-popup-container').css("left")=='0px') {	
			if (response.popup && response.popup == "success") {
				ajaxcartxTools.hidePopup(response.close_popup, false);  
			} else {
				ajaxcartxTools.hidePopup(response.close_popup, true);  				
			}
		} 
		
		if (response.popup) {
			if ((response.popup == "success" && this.showNotification == 1) || response.popup != "success") {
				setTimeout(function() {ajaxcartxTools.showPopup(response.popup,response.is_action);}, ajaxcartx.successPopupDelay);
			}		
		}
	},	
	...
	initAjaxcartx: function(url,button,popup){
		...
		if (popup == 'options' || (!url && jQueryAC('#product_addtocart_form').length > 0)){
			...
			if (!url && jQueryAC('#product_addtocart_form').length > 0){		
				...
				var ajaxcartxProductAddToCartForm = productAddToCartForm;							
				var addToCartUrl = this.initUrl;
			} else {
				formId = 'options_addtocart_form';					
				var ajaxcartxProductAddToCartForm = new VarienForm(formId);	

				//if cart page, add param to load new cart page html as well	
				if (ajaxcartx.updateWindow.location.href.indexOf('checkout/cart')!=-1 && jQueryAC('#' + formId + ' #is_cart_page').length == 0) {
				...
				if (window.location.href.indexOf('catalog/product_compare/index/')!=-1 && jQueryAC('#'+ajaxcartx.comparePopup).length > 0 && jQueryAC('#'+formId+' #is_compare_popup').length == 0) {
			...
			if (ajaxcartxProductAddToCartForm.validator.validate() && !this.ajaxCartRunning) {
			...
			if (!this.ajaxCartRunning) {
				...
				//if cart page, add param to load new cart page html as well	
				if (ajaxcartx.updateWindow.location.href.indexOf('checkout/cart')!=-1) {
				...
				if (droppableSidebars['cart'] && !popup && jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar).length>0) {
					ajaxcartxTools.jump(productId, jQueryAC('#'+ajaxcartx.cartSidebar + ajaxcartx.lastCartSidebar), button);
				}
	...
	updateCart: function(response){
		if (response.update_section.html_cart) {
			for (var i=0;i<response.update_section.html_cart.length;i++) {
				if (response.update_section.html_cart[i] && this.updateWindow.document.getElementById(ajaxcartx.cartSidebar+i)) {
		    		...
		    		if ((content.indexOf('<ac-qty>') != -1 || content.indexOf('&lt;ac-qty&gt;') != -1) && !jQueryAC('#'+ajaxcartx.cartSidebar + i).hasClass('ac-header')) { 
			    	    var actionsContainer = '<div class="actions" id="ajaxcartx-actions" style="border-bottom:none;">';
					    actionsContainer += '<button onclick="ajaxcartx.emptyCartConfirmation();" class="button left" title="' + Translator.translate('Empty Cart').stripTags() + '" type="button"><span><span>' + Translator.translate('Empty Cart').stripTags() + '</span></span></button>';
					    if (ajaxcartx.sidebarQty == 1){
					    	actionsContainer += '<button onclick="ajaxcartx.updateQty(\'\',false,\'' + ajaxcartx.cartSidebar + i + '\')" class="button" title="' + Translator.translate('Update Cart').stripTags() + '" type="button"><span><span>' + Translator.translate('Update Cart').stripTags() + '</span></span></button>';
					...
					//extract jquery events
					var bindings = ajaxcartxTools.extractBindings(ajaxcartx.cartSidebar+i);
					
					//flash cart sidebar
			  		if (window.location.href.indexOf('catalog/product_compare/index/')==-1 && jQueryAC('#'+ajaxcartx.comparePopup).length == 0) {
						var cartSidebar = this.updateWindow.document.getElementById(ajaxcartx.cartSidebar+i);	
					    jQueryAC(cartSidebar).hide().fadeIn(300);					
					}
			    	
			    	//update cart sidebar
					ajaxcartxTools.updateSection(content,(ajaxcartx.cartSidebar+i));	

					//re-apply jquery events
					ajaxcartxTools.reapplyBindings(bindings);
	...
	updateCartLink: function(response){
		if (response.update_section.html_cart_link) {
			if (!cartLink) {
	    		jQueryAC('#ac-links a', ajaxcartx.updateWindow.document).each(function() {
					var href = jQueryAC(this, ajaxcartx.updateWindow.document).attr('href');
					if (href != null && href != '' && href.indexOf('/checkout/cart/') != -1) {
						jQueryAC(this, ajaxcartx.updateWindow.document).html(response.update_section.html_cart_link);
					}    			
				});
			} else {
				jQueryAC(cartLink, ajaxcartx.updateWindow.document).html(response.update_section.html_cart_link);
			}
    	}
	},	
	...
	emptyCartConfirmation: function(){
		if(confirm(Translator.translate("Are you sure you would like to remove all the items from the shopping cart?").stripTags())){
			ajaxcartx.emptyCart(this.clearCartUrl);
		}	
	},	
	...
	updateCartPage: function(response){
		if (response.update_section.html_cart_page && this.updateWindow.document.getElementById(ajaxcartx.cartPage)) {
    		ajaxcartxTools.updateSection(response.update_section.html_cart_page,ajaxcartx.cartPage);	
	...
	updateCartItem: function(url, button){ 
		if (!this.ajaxCartRunning) {	
			...
			if (ajaxcartxProductAddToCartForm.validator.validate()) {
				this.ajaxCartRunning = true;
				if (button) {
					this.addToCartButton = button;
				}
	...
	addToWishlist: function(url, popup){
		if (jQueryAC('#product_addtocart_form').length > 0 && !popup){			
			...
			var ajaxcartxProductAddToCartForm = new VarienForm('product_addtocart_form');
			if (!this.ajaxCartRunning) {
				this.ajaxCartRunning = true;
			...
			if (!this.ajaxCartRunning) {	
				...
				if (jQueryAC(('#' + ajaxcartx.wishlistPage), ajaxcartx.updateWindow.document).length > 0) {
				...
				if (droppableSidebars['wishlist'] && !popup && jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar).length>0) {
					ajaxcartxTools.jump(productId, jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar), false);
				}
		...
		if (!this.ajaxCartRunning) {			
			if (button) {
				...
				if (jQueryAC(('#' + ajaxcartx.wishlistPage), ajaxcartx.updateWindow.document).length > 0) {
					...
					//validate qty
					var itemId = ajaxcartxTools.getProductIdFromUrl(url,'item');
			...
			//if wishlist page, add param to load new wishlist page html as well	
			if (jQueryAC(('#' + ajaxcartx.wishlistPage), ajaxcartx.updateWindow.document).length > 0 && jQueryAC('#'+formId+' #is_wishlist_page').length == 0) {
	...
	initAddWishlistItemToCart: function(itemId) {
	   var url = this.generateWishlistItemUrl(itemId).replace("wishlist/index/cart/","ajaxcartx/wishlist/cart/");
    ...
    updateWishlistItems: function() {
		if (!this.ajaxCartRunning) {
			...
			var url = formAction.replace("wishlist/index/update/","ajaxcartx/wishlist/update/");		
	...
	updateWishlist: function(response){	
		if (response.update_section.html_wishlist) {
			for (var i=0;i<response.update_section.html_wishlist.length;i++) {	
				if (response.update_section.html_wishlist[i]) {
		    		if (this.updateWindow.document.getElementById(ajaxcartx.wishlistSidebar + i)) {	
						ajaxcartxTools.updateSection(response.update_section.html_wishlist[i],(ajaxcartx.wishlistSidebar + i));					
			    	} else if(jQueryAC('.sidebar', this.updateWindow.document).length > 0) {
						var sidebar = jQueryAC('.sidebar', this.updateWindow.document);
						sidebar.first().prepend('<div class="ac-wishlist-sidebar" id="'+ajaxcartx.wishlistSidebar+i+'"></div>');
						ajaxcartxTools.updateSection(response.update_section.html_wishlist[i],(ajaxcartx.wishlistSidebar + i));	
					}
					/*
		else if(this.updateWindow.document.getElementById(ajaxcartx.cartSidebar)) {
						var cart = jQueryAC('#'+ajaxcartx.cartSidebar, this.updateWindow.document);
		...
		*/				
					...
					if(i==ajaxcartx.lastWishlistSidebar && (ajaxcartx.dragdropCategory == 1 || ajaxcartx.dragdropCategory == 2) && droppableSidebars['wishlist'] && !jQueryAC('#'+ajaxcartx.wishlistSidebar + ajaxcartx.lastWishlistSidebar).hasClass('ac-header') && !ajaxcartx.isMobile && !ajaxcartx.isTablet) {
						if (window.opener != null && window.location.href.indexOf('catalog/product_compare/index/')!=-1 && jQueryAC('#'+ajaxcartx.comparePopup).length > 0) {
							window.opener.ajaxcartx.makeWishlistDroppableArea();		
						} else {
					    	ajaxcartx.makeWishlistDroppableArea();		
					    }
					}	
					...
					//flash wishlist block
			  		if (window.location.href.indexOf('catalog/product_compare/index/')==-1 && jQueryAC('#'+ajaxcartx.comparePopup).length == 0) {
						var wishlistSidebar = this.updateWindow.document.getElementById(ajaxcartx.wishlistSidebar + i);			    	
	...
	updateWishlistPage: function(response){
		if (response.update_section.html_wishlist_page && this.updateWindow.document.getElementById(ajaxcartx.wishlistPage)) {
    		ajaxcartxTools.updateSection(response.update_section.html_wishlist_page, ajaxcartx.wishlistPage);	
	...
	addToCompare: function(url, popup){
		if (!this.ajaxCartRunning) {
			...
			var productId = ajaxcartxTools.getProductIdFromUrl(url,'product');
			if (droppableSidebars['compare'] && !popup && jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar).length>0) {
				ajaxcartxTools.jump(productId, jQueryAC('#'+ajaxcartx.compareSidebar + ajaxcartx.lastCompareSidebar), false);
			}
	...
	updateCompare: function(response){
		if (response.update_section.html_compare) {
			for (var i=0;i<response.update_section.html_compare.length;i++) {
				if (response.update_section.html_compare[i] && this.updateWindow.document.getElementById(ajaxcartx.compareSidebar+i)) {
					
					//extract jquery events
					var bindings = ajaxcartxTools.extractBindings(ajaxcartx.compareSidebar+i);

		    		ajaxcartxTools.updateSection(response.update_section.html_compare[i],(ajaxcartx.compareSidebar+i));
					...
					//flash compare block
			  		if (window.location.href.indexOf('catalog/product_compare/index/')==-1 && jQueryAC('#'+ajaxcartx.comparePopup).length == 0) {
						var compareSidebar = this.updateWindow.document.getElementById(ajaxcartx.compareSidebar+i);
	...
	updateComparePopup: function(response){
		if (response.update_section.html_compare_popup && jQueryAC('#'+ajaxcartx.comparePopup).length > 0) {
    		ajaxcartxTools.updateSection(response.update_section.html_compare_popup,ajaxcartx.comparePopup);

			//flash compare popup block
			jQueryAC('#'+ajaxcartx.comparePopup).hide().fadeIn(300);	
	    }
	},
